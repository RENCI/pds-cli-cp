...
def get_resource(resource_type, patient_id, fhir):
    logger.info(f"********************** get reousce {resource_type} {patient_id}")
...
def get_patient(patient_id, fhir):
    logger.info("*********************************get_patient " + str(fhir))
...
def birthTime(patient):
    if patient == None:
        ...
        if "birthTime" in patient:
            return Right(patient["birthTime"])
        else:
            return Right({
                "variableValue": {
                    "value": "00:00"
...
gender2sex = {
    "male": "M",
    "female": "F",
    "unknown": "NI", # can also be UN, but that doesn't show up  in COVID19 PCORNet extract so we forego it here.
    "other": "OT", # can also be A, but we see neither OT nor A in real data
}
...
def sex(patient):
    if patient == None:
        ...
        if "gender" in patient:
            return Right({
                "variableValue": {
                    "value": gender2sex.get(patient["gender"],"NI")
                }
            })
        else:
            return Right({
